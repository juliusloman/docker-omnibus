...
#
if (defined $FORM{'NmosSerial'}) {
  $URL = "../eventviewer/eventViewer.jsp?transientname=ShowRootCause&viewname=NetworkManagementEvents&viewtype=global&sql=ServerSerial%3D$FORM{'NmosSerial'}%20AND%20ServerName%3D'$FORM{'ServerName'}'";
} else {
  $URL = "../eventviewer/eventViewer.jsp?transientname=ShowRootCause&viewname=NetworkManagementEvents&viewtype=global&sql=ServerSerial%3D22";
}
...
print STDOUT "<html>\n";
print STDOUT "<head>\n";
if (defined $FORM{'NmosCauseType'} and $FORM{'NmosCauseType'} == 2) {
  ...
  print STDOUT "This is not a suppressed event (NmosCauseType = 2)\n";
  print STDOUT "<pre>\n";
